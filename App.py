...
pyrebase:login
firebase_admin:db,regsitration
...
"""
Items marked by 'Sample to be edited' need to be user modified as in service-account.json file
"""
...
#Sample to be edited
dbURL = "https://<web-name>.firebasedatabase.app/"

firebaseConfig = {
    "apiKey": "Sample to be edited",
    "authDomain": "Sample to be edited",
    'databaseURL': dbURL,
    "projectId": "Sample to be edited",
    "storageBucket": "Sample to be edited",
    "messagingSenderId": "Sample to be edited",
    "appId": "Sample to be edited",
    ...
    def start(self,grab=True):
        ...
        try:
            if not LOADING_SCREENS:
                ...
                screen.geometry("+%d+%d" % (screen_width // 2 - gifhalfdimension[0], screen_height  - 3*gifhalfdimension[1]))
                screen.lift()
...
class Apptools:
    ...
    def clearImgCache():
        ...
        onlyfiles = [os.path.join(CACHE_FOLDER,f) for f in os.listdir(CACHE_FOLDER) if os.path.isfile(os.path.join(CACHE_FOLDER, f))]
        for l in onlyfiles:
            if os.path.exists(l):
                os.remove(l)
...
class FirebaseDB:
    ...
    def create_account(email,password,username,details,table):
        if FirebaseDB.connect():
            ...
            messagebox.showwarning(title="Warning!",message="Failed to Connect to Server\nNo Internet Connection or Server Unreachable")
    ...
    def updateData(child,data,identifier="Email",identifierval="",showWarning=True):
        """
        In case of no identifier,function will update values for current user
        """
        try:
            if FirebaseDB.connect():
        ...
        try:
            if FirebaseDB.connect():
                ...
                if fileLocation and saveAsName:
                    revfn = Apptools.rev(fileLocation)
                    extension = ".png"
    ...
    def getDataStorage(imgURL,showWarning=True):
        try:
            if FirebaseDB.connect():
                ...
                imgLocation = os.path.join(CACHE_FOLDER,"{0}.png".format(formatted_date+str(random.randint(0,1000))))
    ...
    def optimiseSearchVal(searchVal):
        if isinstance(searchVal, str) and searchVal:
            x1 = searchVal.title()
            x2 = searchVal.lower()
            x3 = x1[0].lower() + x1[1:]

            return searchVal, x1, x2, x3
    ...
    def searchData(child, identifier, searchVal, filtervar=None,filterval=None,showWarning=True):
        if FirebaseDB.connect():
            try:
                ...
                if filterval and filtervar:
                    res = []
                    [res.append(x) for x in searchResult if x[filtervar]==filterval]
                    searchResult = res
            ...
            if Apptools.check_mail(email):
                try:
                    ...
                    if error == "INVALID_EMAIL":
                        ...
                        messagebox.showwarning("Incorrect Credentials!", "Incorrect Email!\nEnter correct Email address")
                    else:
...
class Apptoolsv2:
    ...
    def itemSearch(self, text, criteria,filtervar= None,filterval=None,showWarning = True,deepSearch=False):
        if Apptools.is_not_null(text):
            if criteria not in ("Wholesale Price","Retail Price"):
                if deepSearch:
                    record = FirebaseDB.deepSearchData("Items", criteria, text, filtervar=filtervar, filterval=filterval,showWarning=showWarning)
                else:
                    record = FirebaseDB.searchData("Items",criteria,text,filtervar= filtervar,filterval=filterval,showWarning=showWarning)
            else:
    ...
    def Treeoutput(self, column, out, label=None, srow=0, scolumn=0, scolumnspan=1,singleLineFilter = True,InScrollableframe=True,lbrow=1,lbcolumn=0,fheight=250,fwidth=750):
        """
        ...
        for i in range(0, len(column)):
            Apptoolsv2.listBox.heading(column[i], text=column[i],command=lambda c=column[i]: Apptoolsv2.sortby(self,Apptoolsv2.listBox, c, 0))
...
class ScrollableFrame(ttk.Frame):
    def __init__(self, container,cw=775,ch=500,showscrlbar=True,*args, **kwargs):
        ...
        self.canvas.config(width=self._canvasWidth,height=self._canvasHeight,scrollregion=(0,0, self._canvasWidth, self._canvasHeight))
        if showscrlbar:
        ...
        if FirebaseDB.connect():
            ...
            if session:
                try:
                    ...
                    choice = messagebox.askyesno("Alert", "Detected a previous session!\nWant to Continue Previous Session?")
...
class Page2_SignupAdmin(tk.Frame):
    ...
    def makeWidgets(self, master):
        ...
        button.config(command=lambda: self.Processing(master, frame.scrollable_frame, name.get(), Email.get(), username.get(), Password.get(), Repassword.get(), Pin.get()))
                ...
                if FirebaseDB.connect():
                    ...
                    if not (pre_user1 or pre_user2 or pre_user3):
                        ...
                        if walno:
                            ...
                            if status==True:
                                data={"WalletNo":walno,"Email":email,"UserType":"Admin","Balance":0,"PIN":pin}
...
class Page2_SignupBuyer(tk.Frame):
    ...
    def makeWidgets(self, master):
        ...
        button = tk.Button(frame.scrollable_frame, text="Register")
        button.config(command=lambda: self.Processing(master, name.get(), emailid.get(), username.get(), Password.get(), Repassword.get(), Pin.get(), DelAdd.get("1.0", "end-1c")))
        button.config(bg="#1F8EE7", fg="#E8E8E8", bd=0, activebackground="#3297E9")
        button.config(padx=5,pady=3)
            ...
            if cond1 and cond2 and cond3:
                ...
                if FirebaseDB.connect():
                    ...
                    if not (pre_user1 or pre_user2 or pre_user3):
                        walno = Apptools.generateuniquecode(WALLETTABLE, "WalletNo")
                        if walno:
                            details = {"Name": name, "Email": email, "Username": user, "PIN": pin, "Delivery Address": DelAdd.strip(),
                                       "Premium Account": "No", "WalletNo": walno}
                            status=statusPush=False
                            status = FirebaseDB.create_account(email, pswd, user, details, table)

                            if status==True:
                                data = {"WalletNo": walno, "Email": email, "UserType": "Buyer", "Balance": 0, "PIN": pin}
                                statusPush = FirebaseDB.pushData(WALLETTABLE, data)
                    ...
                    if not (pre_user1 or pre_user2 or pre_user3):
                        ...
                        if walno:
                            details = {"Name": name, "Email": email, "Username": user, "PIN": pin,
                                       "Organisation Name": OrgName, "Organisation Address": OrgAdd.strip(), "WalletNo": walno}
                            status=statusPush=False
                            status = FirebaseDB.create_account(email, pswd, user, details, table)
                            if status==True:
                                data = {"WalletNo": walno, "Email": email, "UserType": "Buyer", "Balance": 0, "PIN": pin}
                            ...
                            if ref3:
                                ...
                                if cash!=0:
                                    ...
                                    if choice:
                                        ...
                                        if step1:
                                            step2 = FirebaseDB.deleteData("Wallet",identifier="Email",value=SESSION_USER.email)
                                            if step2:
                                                step3 = FirebaseDB.deleteData("Cash Record Admin",key=list(ref3.keys())[0])
                                                if step3:
                        ...
                        if ref3:
                            ...
                            if ref4:
                                ...
                                if adminWalNo != walno:
                                    ...
                                    res2 = FirebaseDB.updateData("Cash Record Admin",{"Balance":cashRecordBal-(kbal-5)})
                                    if res1 and res2:
                                    ...
                                    messagebox.showerror("Cashout not possible!","You can't take cashout of your own personal account")

                    else:
                        messagebox.showwarning("Invalid Credentials","Either Email Address or Withdrawl amount is incorrect.")
                    ...
                    if ref2:
                        ...
                        if ref3:
                            ...
                            if adminWalNo != walno:
                                ...
                                if choice:
                                    res1 = FirebaseDB.updateData("Wallet", {"Balance": data2['Balance']+int(netamt)},identifierval=email)

                                    if res1:
                                        res2 = FirebaseDB.updateData("Cash Record Admin", {"Balance":
                                                                     cashRecordBal + int(amt)})
                                        if res2:
                                            res3 = FirebaseDB.updateData("Wallet", {"Balance": data['Balance']+5})
                                            if res3:
                                                messagebox.showinfo("Transaction successful!","Transferred Rs. "+netamt+" to user Successfully\nAsk user to check his account")
                                                msg="Transaction Receipt\nEmail : {0}\nNet Amount Recharged : {1}\nPG & Other Charge : 5\nCash Given by user : {2}".format(email,netamt,amt).strip()
                                                sep = ttk.Separator(self, orient='horizontal')
                                                sep.grid(row=6, column=0, columnspan=5, sticky="ew")
...
class Page4_AdminPendingCashout(tk.Frame):
    ...
    def output(self, out):
        ...
        Apptoolsv2.Treeoutput(self, column, out, srow=2,scolumn=1,singleLineFilter=False,InScrollableframe=False,lbrow=2,lbcolumn=1)
...
class Page4_SellerShowProfile(tk.Frame):
    ...
    def check(self,Loadinglbl):
        if FirebaseDB.connect():
            Fieldname=["Name","Username","Email","Name of Organisation","Address of Organisation","Wallet Number"]
            for i in range(len(Fieldname)):
                lbl = tk.Label(self, text=Fieldname[i] + ":")
                lbl.config(font=("Segoe Print", 15), fg="#E8E8E8", bg="#333333")
                lbl.grid(row=i + 2, column=1, padx=5)
                                ...
                                if step3 is not None:
                                    ...
                                    if chk:
                                        step4 = FirebaseDB.deleteData("Items",
                                                                      identifier="Seller Email",value=SESSION_USER.email)
                                        if step4:
...
class Page4_SellerAddItems(tk.Frame):
    ...
    def makeWidgets(self, master):
        ...
        btn.config(command=lambda: self.Processing(master,Iname.get(),Iwhp.get(),Irp.get(),Istock.get(),IDesc.get("1.0","end-1c"),CategoryVar.get()))
        btn.config(bg="#1F8EE7",padx=3,fg="#E8E8E8",bd=0,activebackground="#3297E9")
        ...
        if cond1 and cond2 and cond3:
            ...
            if ref:
                ...
                if uploadImgurl:
                    ...
                    if icode:
                        details={"ItemCode":icode,"ItemName":iname,"Wholesale Price":float(iwp),"Retail Price":float(irp),"Category":icat,
            ...
            if cond2 and not(cond3):
                messagebox.showwarning("Invalid Input","Wholesale must be less than or equal to retail price and must be greater than zero.")
            else:
...
class Page6_BuyerProductView(tk.Frame):
    ...
    def makeWidgets(self, master):
        ...
        btn=tk.Button(self, text="Add to Cart &\nProceed to Pay", command=lambda: self.Processing(master, icode, qty.get(), istock))
        btn.config(bg="#1F8EE7",padx=3,fg="#E8E8E8",bd=0,activebackground="#3297E9")
                    ...
                    if res2:
                        ...
                        if res3:
                            ...
                            if prestockuser+int(iqty)<=itemstock:
                                ...
                                rec = FirebaseDB.updateData("BuyerUsers/"+key+"/Cart",data,identifier="ItemCode",identifierval=int(icode))
                                if rec:
                                    if showMsg:
    ...
    def initialisecart(self,master,sframe):
        ...
        if Item:
            ...
            for data in Item:
                ...
                btn=tk.Button(frame.scrollable_frame,text="Remove Item",command=lambda x=icode: self.Processing(master,x,None))
                btn.config(bg="#1F8EE7",padx=3,fg="#E8E8E8",bd=0,activebackground="#3297E9")
        ...
        btn=tk.Button(sframe.scrollable_frame, text="Proceed to Pay", command=lambda: self.payportal(master, Item, price, sframe))
    ...
    def checktrans(self,master,pin,price,out,sframe,screen):
        ...
        if pin==opin and FirebaseDB.connect():
            ...
            if BuyerbalanceInAC>=price+5:
                txt="Transaction Amount : " + str(price+5) +"\nIncluding PG Charges\nBalance after deduction : " + str(round(BuyerbalanceInAC - (price + 5), 2)) + "\nAre you sure want to proceed?"
                    ...
                    if SUPERADMIN:
                        ...
                        if rec:
                            ...
                            for i in range(len(out)):
                                ...
                                rec2 = FirebaseDB.updateData("Wallet",{"Balance": sbalanceinAC + round(((irp*iqty-netbargain)*0.95),2)}, identifierval=sellerEmail)
                                if rec2:
                                    rec3 = FirebaseDB.deleteData("BuyerUsers/"+buyerKey+"/Cart","ItemCode",icode)
                                    if rec3:
                                        rec4 = FirebaseDB.updateData("Items", {"Stock":istock-iqty},identifier="ItemCode",identifierval=icode)
                            ...
                            rec5 = FirebaseDB.updateData("Wallet",{"Balance": admBalanceInAC + 5+round(price*0.05,2)}, identifierval=admEmail)
                            if rec5 is not None:
                                Trec=Trec and rec5
                            else:
    ...
    def getData(self,master):
        ...
        if Item:
            ...
            for data in Item:
                ...
                btn=tk.Button(frame.scrollable_frame, text="Remove Item", command=lambda x=icode: self.Processing(master, x))
                btn.config(bg="#1F8EE7",padx=3,fg="#E8E8E8",bd=0,activebackground="#3297E9")
                btn.grid(row=r, column=1,padx=3)

                btn=tk.Button(frame.scrollable_frame, text="Add to Cart", command=lambda x=(icode, istock): self.addtocart(master,x[0], x[1]))
                btn.config(bg="#1F8EE7",padx=3,fg="#E8E8E8",bd=0,activebackground="#3297E9")
